...
public class OrderService {
    ...
    public void saveMemberOrder(String paymentId, Long memberId,
        ...
        try {
            ...
            int usedCouponPrice = memberFeignClient.getCouponPrice(memberCouponId, orderItemsPrice);

            saveOrder(paymentId, memberId, null, receiverNickname, receiverPhone,
                    zipCode, streetAddress, detailAddress, message, orderItemsPrice, memberCouponId, usedCouponPrice, usedPoint, orderItems);
            itemFeignClient.decreaseStocks(requests);
            memberFeignClient.useCouponAndPoint(memberCouponId);
        } catch (OrderFailException e) {
            //주문은 자동으로 롤백되서 취소 됨
            //주문 취소안하고, DB 장애 해결되면, 관리자 페이지에서 로그 조회하여 주문 재등록하게 하는 방법도 있음 (이벤트 써도 되고)
            log.warn(e.getMessage());
            //OrderFailException은 feign 로직 전에 발생
            ...
            memberFeignClient.rollbackCouponAndPoint(memberCouponId);
            portOneService.cancelPayment(paymentId, e.getMessage());
            throw new WillCancelPaymentException(e.getMessage());
        }
    }
    ...
    public void saveMemberFreeOrder(Long memberId,
        ...
        try {
            ...
            saveOrder(null, memberId, null, receiverNickname, receiverPhone,
                    zipCode, streetAddress, detailAddress, message, orderItemsPrice, memberCouponId, usedCouponPrice, usedPoint, orderItems);
            itemFeignClient.decreaseStocks(requests);
            memberFeignClient.useCouponAndPoint(memberCouponId);
            //무료 주문이라 포인트 적립 없음
    ...
    public Long saveGuestOrder(String paymentId,
        ...
        try {
            ...
            return saveOrder(paymentId, null, guest, receiverNickname, receiverPhone,
                    zipCode, streetAddress, detailAddress, message, orderItemsPrice, null, 0, 0, orderItems);
        } catch (OrderFailException e) {
            log.warn(e.getMessage());
            itemFeignClient.increaseStocks(requests);
            portOneService.cancelPayment(paymentId, e.getMessage());
    ...
    public void cancelMemberOrder(Long orderId, Long memberId, String jwt, String reason) {
        Order order = orderRepository.findById(orderId)
                .orElseThrow(() -> new IllegalArgumentException(NOT_FOUND_ORDER));

        boolean isOwner = order.getMemberId().equals(memberId);
        Authority authority = memberFeignClient.findAuthority(jwt);
    ...
    public void cancelMemberFreeOrder(Long orderId, Long memberId, String jwt) {
        Order order = orderRepository.findById(orderId)
                .orElseThrow(() -> new IllegalArgumentException(NOT_FOUND_ORDER));
    ...
    private void validateMemberOrderPrice(int orderItemsPrice, Long memberCouponId, Integer usedPoint, Integer clientTotal, Long memberId) {
        ...
        int orderPriceUsedCoupon = orderItemsPrice - memberFeignClient.getCouponPrice(memberCouponId, orderItemsPrice);
